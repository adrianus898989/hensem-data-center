-- Private classification page: compact history and optimistic mapping versions.
begin;
...
begin
  ...
  with mapped as materialized (
    select m.id,m.team_name,m.system_name,m.source_system,m.country_name,m.country_code,
      m.source_country,m.platform_name,m.source_platform,m.active,md5(to_jsonb(m)::text) as version
    ...
    select v.country,v.platform,
      sum(v.matched_count)::bigint as matched_count,
      sum(v.charge_count)::bigint as charge_count,
      sum(v.withdraw_count)::bigint as withdraw_count,
      max(v.last_data_date) as last_data_date,max(v.updated_at) as updated_at
    from private.dashboard_admin_provider_registry v
    ...
    select m.id,m.team_name,m.system_name,m.source_system,m.country_name,m.country_code,
      ...
      true as mapped,
      case when a.platform is null then 'no_data' else 'mapped' end as status,m.version
    ...
    select md5('unmapped:'||a.country||':'||a.platform)::uuid,
      ...
      false,'unmapped',md5(jsonb_build_array('unmapped',a.country,a.platform)::text)
    from actual a
  ...
  select jsonb_build_object(
    'version',2,'canManage',private.dashboard_admin_live_can_configure(),'canGrant',exists(select 1 from public.dashboard_profiles where auth_user_id=auth.uid() and active and role='owner'),'basis','dashboard_platform_team_map + third_party_volume',
    ...
    'options',jsonb_build_object(
      'sourceSystems',(select jsonb_agg(x.source_system order by x.source_system) from (select distinct source_system from mapped) x),
      'countryChoices',(select jsonb_agg(to_jsonb(x) order by x.country) from (select distinct country_name as country,country_code as code from mapped) x),
    ...
    'rows',coalesce((select jsonb_agg(jsonb_build_object(
      ...
      'version',p.version,'source','Supabase · dashboard_platform_team_map + third_party_volume')
