-- Read the explicitly labelled INR account amount in legacy India USDT recharge cells.
-- Never multiply USDT by a rate or mix the separate USDT value into local totals.
-- Accept only the complete known three-label format; preserve invalid/missing values.
-- No source record, public API, auth, scope or privilege changes.
    ...
    v_sql:=format($q$
      select md5(jsonb_build_array(a.source_system,a.country_code,a.platform,a.order_kind,a.order_no)::text)::uuid as id,
        ...
        coalesce(a.amount,case
          ...
          then btrim(a.amount_text)::numeric
          when a.country_code='IN' and a.order_kind='recharge' and btrim(a.raw_channel) ~ '^USDT[(]TRC20[)]-[0-9]+$'
            and length(a.amount_text)<=250
          then substring(replace(a.amount_text,chr(92)||'n',chr(10)) from
            '^[[:space:]]*金额[：:][[:space:]]*([0-9]{1,18}([.][0-9]{1,8})?)[[:space:]]+兑换比例[：:][[:space:]]*[0-9]+([.][0-9]+)?[[:space:]]+USDT[：:][[:space:]]*[0-9]+([.][0-9]+)?[[:space:]]*$')::numeric end) as amount,null::numeric as actual_amount,null::numeric as withdraw_fee,a.updated_at as synced_at,$22::text as currency,
        a.amount_text
