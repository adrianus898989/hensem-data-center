-- India legacy AR read contract. No order/config writes, no FX conversion.
-- AR country IN stores platform-account amounts in INR, including USDT rails;
-- its old config registry omitted currency. Preserve every explicit currency.
-- Signed manual-recharge adjustments survived in amount_text while the old
-- non-negative numeric parser stored NULL. Read only strict negative decimals
-- for this confirmed India/manual/recharge source, never guess other missing money.
-- Keep existing auth/scope checks, private functions, grants and catalog index probes.
...
begin
  ...
  select md5('ar:'||t.country_code||':'||t.platform)::uuid,t.platform,t.country_name,t.country_name,
    t.country_code,'ar'::text,t.timezone,
    coalesce(t.currency,case when t.country_code='IN' then 'INR' end)
      ...
      select md5(jsonb_build_array(a.source_system,a.country_code,a.platform,a.order_kind,a.order_no)::text)::uuid as id,
        ...
        coalesce(a.amount,case
          when a.country_code='IN' and a.order_kind='recharge' and btrim(a.raw_channel)='人工充值'
            and length(btrim(a.amount_text))<=24 and btrim(a.amount_text) ~ '^-[0-9]+([.][0-9]+)?$'
          then btrim(a.amount_text)::numeric end) as amount,null::numeric as actual_amount,null::numeric as withdraw_fee,a.updated_at as synced_at,$22::text as currency,
        a.amount_text
