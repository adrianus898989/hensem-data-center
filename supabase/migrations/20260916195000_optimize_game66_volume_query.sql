-- Keep the 66GAME daily/team volume RPC within the dashboard request budget.
-- The source table deliberately stores the raw vendor response, making heap
-- rows wide.  This covering index lets the aggregate read only the compact
-- fields it needs and keeps the local-date range sargable.
create index if not exists game66_charge_orders_volume_cover_idx
  on public.game66_charge_orders (platform_id, create_time)
  include (
    pay_method_name,
    pay_method_code,
    channel,
...
begin
  ...
  -- GAME66 timestamps are displayed in Asia/Kolkata. Convert the two date
  -- boundaries once instead of casting every source row in the WHERE clause.
