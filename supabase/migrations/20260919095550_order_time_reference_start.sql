-- Preserve the original selected start when the frontend reads a long range in
-- small time slices. Only earlier_count/earlier_amount use this reference;
-- time, status, provider and permission filters remain unchanged.
-- Remove the old signature to keep PostgREST resolution unambiguous. Calls
-- supplying the old eleven named arguments still work through the new default.
drop function if exists public.dashboard_order_time_query(uuid,timestamptz,timestamptz,text,text,timestamptz,timestamptz,text,text,text,boolean);
drop function if exists private.dashboard_order_time_query(uuid,timestamptz,timestamptz,text,text,timestamptz,timestamptz,text,text,text,boolean);
...
declare
  ...
  v_reference_start timestamptz := coalesce(p_reference_start,p_start_at);
begin
  ...
  if p_basis is null or p_basis not in ('created','success') or p_direction is null or p_direction not in ('all','charge','withdraw')
    ...
    or not isfinite(v_reference_start) or v_reference_start > p_start_at
    or p_end_at-v_reference_start > interval '31 days'
  ...
  execute format($query$
    with orders as (
      ...
      select direction,provider,channel_type,
        ...
        count(*) filter(where succeeded and create_time < $12) as earlier_count,
        coalesce(sum(amount) filter(where succeeded and create_time < $12),0) as earlier_amount,
  ...
  into v_rows using p_platform,p_start_at,p_end_at,p_direction,p_basis,p_created_start,p_created_end,
    v_member_id,v_order_number,p_status,p_cross_day_only,v_reference_start;
...
create or replace function public.dashboard_order_time_query(
  ...
  select private.dashboard_order_time_query(p_platform,p_start_at,p_end_at,p_basis,p_direction,p_created_start,p_created_end,p_member_id,p_order_number,p_status,p_cross_day_only,p_reference_start);
$$;
...
revoke all on function public.dashboard_order_time_query(uuid,timestamptz,timestamptz,text,text,timestamptz,timestamptz,text,text,text,boolean,timestamptz) from public,anon;
...
grant execute on function public.dashboard_order_time_query(uuid,timestamptz,timestamptz,text,text,timestamptz,timestamptz,text,text,text,boolean,timestamptz) to authenticated;
