-- Keep both greatest() operands numeric on Postgres 17.
...
begin
  ...
  with grouped as (
    select
      ...
      coalesce(avg(pg_catalog.greatest(0::numeric, extract(epoch from (coalesce(w.update_time, w.submit_time, w.last_seen_at, w.create_time) - w.create_time)))) filter (where w.create_time is not null), 0)::numeric as avg_seconds,
