...
-- Same confirmed rail/code pairs as workOrderDepositThirdPartyName in the
-- existing dashboard. A payment rail alone must not become a guessed provider.
...
$$;
revoke all on function private.dashboard_admin_live_workorder_provider(text,text,text,text) from public,anon,authenticated;
...
begin
  ...
  with targets as materialized (
    select distinct p.name,p.source_name,p.country from private.dashboard_admin_live_platforms() p
  ...
  select jsonb_build_object(
    ...
    'coverage',jsonb_build_object('expectedPlatformDays',coalesce((select sum(expected_days) from coverage),0),
      ...
      'platforms',coalesce((select jsonb_agg(jsonb_build_object('platform',platform,'sourcePlatform',source_platform,
