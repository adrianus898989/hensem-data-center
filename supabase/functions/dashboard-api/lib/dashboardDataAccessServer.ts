...
export function dashboardDataErrorResponse(error: unknown): Response {
  ...
  return new Response(JSON.stringify({ok: false, code: known ? error.code : "data_unavailable",
    message: known ? error.message : "数据读取暂时不可用，请稍后重试。"}), {
    status: known ? error.status : 503,
    headers: {...dashboardPrivateHeaders(), "Content-Type": "application/json; charset=utf-8"},
  });
}
