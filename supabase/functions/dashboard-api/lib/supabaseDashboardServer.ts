...
export async function readSupabaseThirdPartyVolume(request: Request, startInput = "", endInput = "", countryInput = ""): Promise<ThirdPartyVolumePayload> {
  ...
  // UI page labels include the “盘口” suffix, while the database stores the
  // canonical team code/name. Passing the display label made the GAME66 RPC
  // return a valid but empty payload, so the dashboard incorrectly showed 0.
  ...
  // Keep the comparison day out of the main GAME66 aggregation. A two-day raw
  ...
  // Team pages prioritise a complete current result. Their previous-day raw
  // history may contain hundreds of thousands of orders and must not block it.
  ...
  const game66Read = Promise.all([game66CurrentRead, game66PreviousRead]).then(([current, previous]) => ({
    rows: current.rows || [],
    collectionSuccessSnapshots: [
      ...(previous.collectionSuccessSnapshots || []),
      ...(current.collectionSuccessSnapshots || []),
    ],
    withdrawPendingSnapshots: [
      ...(previous.withdrawPendingSnapshots || []),
      ...(current.withdrawPendingSnapshots || []),
    ],
