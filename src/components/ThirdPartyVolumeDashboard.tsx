...
export default function ThirdPartyVolumeDashboard() {
  ...
  const countryPageMonthlyRows = useMemo(() => {
    ...
    return [...existing, ...missing, ...pendingMissing, ...depositMissing, ...actualMissing];
  }, [countryPageRows, collectionSuccess, withdrawPending, workOrderDeposit, withdrawActual, appliedDirection]);
...
function MonthlyTable({ title, subtitle, rows, columns, feeRows = [], paginated, compact, collectionSuccess, withdrawPending, workOrderDeposit, withdrawActual }: { title: string; subtitle: string; rows: ComboSummary[]; columns: string[]; feeRows?: FeeCompareRow[]; paginated?: boolean; compact?: boolean; collectionSuccess?: CollectionSuccessView; withdrawPending?: WithdrawPendingView; workOrderDeposit?: WorkOrderDepositView; withdrawActual?: WithdrawActualView }) {
...
function WithdrawActualCell({ metric, kind }: { metric: { actualAmount: number; feeAmount: number; state: string } | undefined; kind: "actual" | "fee" }) {
  ...
  return <span className={`withdraw-actual-value ${ready ? "is-ready" : "is-pending"}`} title={`${label}来自 game66_withdraw_orders 的安全汇总；来源覆盖不完整时不显示 0。`}>{ready ? formatNumber(value || 0) : "—"}</span>;
}
