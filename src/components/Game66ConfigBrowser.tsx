...
export default function Game66ConfigBrowser({team}:{team:"hong_kong"|"red_crab"}) {
  ...
  useEffect(()=>{const controller=new AbortController();setLoading(true);setError("");setRows([]);setTargets([]);setRules([]);
    if(!session||!allowed){setLoading(false);return()=>controller.abort();}
    Promise.all([fetchGame66PlatformStatus(session,controller.signal),fetchGame66Config(session,controller.signal)])
      .then(([status,config])=>{if(!controller.signal.aborted){setRows(status.filter(row=>row.team_code===team));setTargets(config.targets.filter(row=>row.team_code===team));setRules(config.rules);}})
  ...
  const ready=rows.filter(row=>row.has_base_url).length;
    ...
    {loading?<div className="awc-empty" role="status">正在读取平台与规则配置…</div>:error?<div className="awc-empty" role="alert">{error}</div>:rows.length===0?<div className="awc-empty">当前权限下没有该团队的平台</div>:
      <div className="awc-workspace"><aside className="awc-platforms"><div className="awc-list-title"><strong>平台配置</strong><span>可同步 {ready}/{rows.length} · 规则 {synced}/{rows.length}</span></div>
        <div className="awc-platform-list">{rows.map(row=>{const config=targets.find(item=>item.platform_name.toUpperCase()===row.platform_name.toUpperCase());return <button type="button" key={row.platform_name} className={selected?.platform_name===row.platform_name?"active":""} onClick={()=>setSelectedName(row.platform_name)}>
          <span>{row.platform_name}</span><small className={config?.rule_count?"fresh":""}>{config?.rule_count?`${config.rule_count} 条规则`:row.enabled?"待采规则":row.request_configured?"未启用":"待配置"}</small></button>;})}</div></aside>
        {selected&&<section className="awc-detail"><header className="awc-detail-head"><div><span className="awc-eyebrow">66GAME / 只读配置</span><h2>{selected.platform_name}<span>{selected.team_name}</span></h2></div>
          ...
          <div className="g66-status-grid"><div><span>连接地址</span><strong>{selected.has_base_url?"已登记":"未登记"}</strong></div><div><span>采集方式</span><strong>{selected.has_base_url?"浏览器会话":"待配置"}</strong></div><div><span>同步规则</span><strong>{selected.request_configured?"已配置":"本机模板"}</strong></div><div><span>最近同步</span><strong>{selected.latest_sync_status||"尚无记录"}</strong></div><div><span>代收订单</span><strong>{selected.charge_rows.toLocaleString("en-US")}</strong></div><div><span>提现订单</span><strong>{selected.withdraw_rows.toLocaleString("en-US")}</strong></div></div>
          <div className="awc-capture-time">代收更新：{stamp(selected.latest_charge_at)}<span>提现更新：{stamp(selected.latest_withdraw_at)}</span><span>规则更新：{stamp(target?.updated_at||null,target?.timezone)}</span></div>
