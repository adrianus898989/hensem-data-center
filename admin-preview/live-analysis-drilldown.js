...
(function(root){
 ...
 function create(c){
  ...
  function sync(){const next=signature();if(next!==scope){scope=next;generation++;active++;states.clear();sortRegistry.clear()}return scope}
  ...
  const sortRegistry=new Map(),number=v=>finite(v)?Number(v):null,fraction=(n,d)=>finite(n)&&finite(d)&&Number(d)>0?Number(n)/Number(d):null;
  const numeric=key=>({value:r=>number(r[key])}),textual=key=>({value:r=>r[key]||null,ascending:true});
  function sortGrid(s,id,headers,rows,columns){
   sortRegistry.set(JSON.stringify([s.segment,id]),columns);
   return root.HensemProviderSummary?.sortableTable?root.HensemProviderSummary.sortableTable({headers,rows,columns,sort:s.sorts?.[id],onSort:index=>action(s.segment,'sort',JSON.stringify([id,index]))}):{headers,rows};
  }
  function metricSortColumns(latency,total){return latency?[numeric('success_amount'),numeric('success_count'),{value:r=>fraction(r.success_amount,total(r).success_amount)},{value:r=>fraction(r.success_count,total(r).success_count)}]:['all_amount','all_count','success_amount','success_count','pending_amount','pending_count','failed_amount','failed_count'].map(numeric).concat({value:r=>fraction(r.success_count,r.all_count)})}
  ...
  function platformGrid(s,aligned=false){
   ...
   rows=root.HensemProviderSummary?.sortedRows?root.HensemProviderSummary.sortedRows(rows,r=>number(r.success_amount)):rows.sort((a,b)=>Number(b.success_amount||0)-Number(a.success_amount||0)||String(a.platform).localeCompare(String(b.platform))||String(a.source).localeCompare(String(b.source)));
   const extraDirection=dirs(s.segment).length>1,hasAction=multi()||aligned;
   const headers=['平台','包网',...(extraDirection?['方向']:[]),...metricHeaders(latency),...(hasAction?['每日对比']:[])],columns=[textual('platform'),textual('source'),...(extraDirection?[textual('direction')]:[]),...metricSortColumns(latency,r=>by[r.direction]),...(hasAction?[null]:[])],sorted=sortGrid(s,'platform',headers,rows,columns);
   return {headers:sorted.headers,rows:sorted.rows.map(r=>[E(r.platform),E(r.source),...(extraDirection?[name(r.direction)]:[]),...metricCells(r,by[r.direction],latency),...(hasAction?[multi()?'<button class="link" onclick="'+action(s.segment,'platformDaily',r.platformId)+'">查看每天</button>':'']:[])])};
  }
  ...
  function providerDayPanel(s){
   ...
   const sorted=sortGrid(s,'provider-day',headers,rows,[textual('platform'),textual('source'),...['all_amount','all_count','success_amount','success_count'].map(numeric),{value:r=>fraction(r.success_count,r.all_count)}]);
   const status=L.queryFailures?.length?'<p class="analysis-status">当前有 '+C(L.queryFailures.length)+' 个平台未读取，以下仅展示已读平台。</p>':'';
   return '<div class="analysis-drilldown analysis-provider-day"><div class="analysis-drilldown-head"><strong>'+E(segment.date+' · '+segment.provider+' · '+(segment.source||'未提供包网')+' · '+name(segment.direction))+'</strong><button class="link" onclick="'+action(segment,'toggle')+'">收起</button></div>'+status+smallTable(sorted.headers,sorted.rows.map(row=>[E(row.platform),E(row.source),N(row.all_amount),C(row.all_count),N(row.success_amount),C(row.success_count),R(row.success_count,row.all_count)]))+(rows.length?'':'<p class="analysis-status">当前已读数据中没有该三方、日期、包网和方向的平台明细。</p>')+'<div class="analysis-note">仅此三方、此日期、此包网和此方向；复用已读取的每日汇总。全部金额、笔数按创建日期；成功金额、笔数按成功日期。成功率 = 当日成功笔数 ÷ 当日创建笔数，跨日成功可能超过 100%；没有创建笔数时显示 —。</div></div>';
  }
  ...
  function dailyGrid(s,aligned=false){
   ...
   const records=rows.map(r=>{const previous=rows.find(p=>p.direction===r.direction&&p.date===new Date(Date.parse(r.date+'T00:00Z')-86400000).toISOString().slice(0,10));return {...r,_delta:r.complete&&previous?.complete&&finite(r.success_amount)&&Number(previous.success_amount)>0?(Number(r.success_amount)-Number(previous.success_amount))/Number(previous.success_amount):null,_coverage:r.covered?(r.complete?'已返回':r.present+' / '+r.expected+' 平台有记录'):'未返回记录'}});
   const headers=['日期',...(extra?['方向']:aligned?['数据状态']:[]),...metricHeaders(latency),'成功金额较前日'],columns=[textual('date'),...(extra?[textual('direction')]:aligned?[textual('_coverage')]:[]),...metricSortColumns(latency,r=>by[r.direction]).map(col=>({value:r=>r.covered?col.value(r):null})),numeric('_delta')],sorted=sortGrid(s,'daily:'+s.platform,headers,records,columns);
   const rendered=sorted.rows.map(r=>[E(r.date)+(aligned?'':'<small class="analysis-metric-share">'+r._coverage+'</small>'),...(extra?[name(r.direction)]:aligned?[r._coverage]:[]),...(r.covered?metricCells(r,by[r.direction],latency):metricHeaders(latency).map(()=> '—')),r._delta===null?'—':(r._delta*100).toFixed(2)+'%']);
   return {before:control+progress+failures,headers:sorted.headers,rows:rendered,after:'<div class="analysis-note">按各平台当地日期；占比以当前平台范围、当前段、同方向的已读日期合计为分母。没有返回记录的日期显示 —，不补成 0；平台记录不齐时不比较增减。首尾日期按所选起止时间，可能不足整日。</div>'};
  }
