...
(function(root){
 ...
 root.HensemLiveReportData={normalizeIdentity,identityKey:keyFor,create({L,E,N,C,R,request,render}){
  ...
  function providerAllowed(row,country){const wanted=S.scope?.providers||[];return !wanted.length||wanted.some(name=>providerName(name,country)===providerName(row.provider,country))}
  ...
  function mergeProviders(rows,country){const groups=new Map();for(const row of rows||[]){const name=providerName(row.provider,country);if(!groups.has(name))groups.set(name,[]);groups.get(name).push(row)}return [...groups].map(([provider,items])=>items.length===1?{...items[0],provider}:{provider,records:items.reduce((sum,r)=>sum+Number(r.records||0),0),metrics:metricSum(items),metricCoverage:Object.fromEntries(Object.keys(metricLabels).map(key=>[key,items.reduce((sum,r)=>sum+Number(r.metricCoverage?.[key]??(r.metrics?.[key]!=null?r.records||0:0)),0)]))})}
  function filteredGroup(group,country=S.scope?.country){if(!S.scope?.providers?.length)return group;if(group.grain!=='provider')return {...group,providerFilterUnsupported:true};const providers=mergeProviders(group.providers,country).filter(r=>providerAllowed(r,country)),daily=(group.daily||[]).map(day=>{const rows=mergeProviders(day.providers,country).filter(r=>providerAllowed(r,country));return {...day,providers:rows,metrics:metricSum(rows),records:rows.reduce((sum,r)=>sum+Number(r.records||0),0)}}).filter(day=>day.records>0);return {...group,providers,daily,metrics:metricSum(providers),records:providers.reduce((sum,r)=>sum+Number(r.records||0),0),metricCoverage:undefined}}
  ...
  function providerTable(group,country){const list=mergeProviders(group.providers,country);if(!list.length)return '<div class="report-note">源日报未提供三方明细。</div>';const metrics=Object.keys(metricLabels).filter(k=>summaryMetrics.includes(k)||list.some(r=>r.metrics?.[k]!=null));return table(['三方',...metrics.map(k=>E(metricLabels[k])+' / 占比')],list.map(row=>[E(row.provider),...metrics.map(k=>metric(row,k)+share(row,group,k))]));}
  function dayTable(group,country){const days=group.daily||[];if(!days.length)return '<div class="report-note">源日报未提供每日明细。</div>';const metrics=Object.keys(metricLabels).filter(k=>summaryMetrics.includes(k)||days.some(r=>r.metrics?.[k]!=null));return table(['源日期',...metrics.map(k=>E(metricLabels[k])),'当日三方'],days.map(day=>[E(day.date),...metrics.map(k=>metric(day,k)),day.providers?.length?'<details class="report-day-providers"><summary>查看 '+C(mergeProviders(day.providers,country).length)+' 个三方</summary>'+providerTable(day,country)+'</details>':'—']));}
  ...
  function capture(){return {result:S.result,scope:S.scope,scopeKey:S.scopeKey,error:S.loading?'日报读取已暂停，点击查询继续':S.error,expanded:new Set(S.expanded),tabs:new Map(S.tabs),completed:S.result?completed:null};}
  function restore(saved){S.serial++;loadPending=null;S.loading=false;S.result=saved?.result||null;S.scope=saved?.scope||null;S.scopeKey=saved?.scopeKey||'';S.error=saved?.error||'';S.expanded=new Set(saved?.expanded||[]);S.tabs=new Map(saved?.tabs||[]);completed=saved?.completed||null;}
  return {loadCatalog,catalog,selected,load,cancel,capture,restore,providers,render:renderReport,state:S};
 }};
