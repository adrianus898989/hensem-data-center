...
- Aggregate requests use the selected interval as one query, with at most four platform workers (matching the bridge concurrency limit); only an actual timeout bisects that interval and retries the smaller ranges. Results are published only after every partition succeeds. Changed partition counts reject a stale details page. Quantiles are not averaged across partitions.
...
Reason buttons open a right-side drawer. Rejection categories use the M8 `arwd.py` headings for India, Pakistan and Nigeria, plus all 53 untagged templates for Brazil, Indonesia, Myanmar, Malaysia and Vietnam. Matching is shared across platforms within the source country, not restricted to the validation sample. Complete templates can match notes containing a shortened UI preview followed by the full tooltip; matching normalizes formatting and the Myanmar renderer's omitted combining dot. Truncated fragments and text matching multiple different categories stay unclassified. Full raw notes remain available; unknown text and blank notes remain distinct. Category, exact-note and operator drilldowns retain the full daily rejected-order denominator across all pages. Order details show the original rejection note, separate interception text, account, order number, amount and timestamps. Categorization never marks an order or operator as wrong. Sources with only aggregate snapshots explicitly disable unavailable order/operator details.
