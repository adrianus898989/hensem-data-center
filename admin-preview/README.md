...
- Aggregate requests use the selected interval as one query, with at most two platform workers; only an actual timeout bisects that interval and retries the smaller ranges. Results are published only after every partition succeeds. Changed partition counts reject a stale details page. Quantiles are not averaged across partitions.
