...
(function(){
 ...
 function quality(){ensureTypes();const nav=choose([['business','三方稳定性与处理表现'],['distribution','三方全部订单分布 / 已知与待接入']]);const rows=combine(groupRows('provider'),['provider','source','direction']);return totals()+nav+(L.view==='distribution'?dirs().map(d=>box(name(d)+' · 三方全部订单分布',bars(rows.filter(r=>r.direction===d).map(r=>({name:E(r.provider)+' · '+E(r.source),value:r.all_count}))))).join(''):box('三方稳定性与处理表现',sortedPageTable('provider-quality',['三方','类型','包网来源','方向','全部金额','全部笔数','成功率','成功完成 P95','掉单金额','掉单笔数','异常金额','异常笔数','笔数份额','估算手续费'],rows,[...providerSortPrefix(),numeric('all_amount'),numeric('all_count'),ratio('success_count','all_count'),...Array(5).fill(null),{value:r=>sorting.fraction(r.all_count,stat(r.direction).all_count)},feeSort],r=>[E(r.provider),typeCell(r),E(r.source||'—'),name(r.direction),N(r.all_amount),C(r.all_count),(R(r.success_count,r.all_count)),...Array(5).fill(unavailable),R(r.all_count,stat(r.direction).all_count),feeForRow(r)]),'P95 与核对标记未接入，保留独立列'))}
 ...
 function daily(){
  ...
  const grid='<div class="live-table table-wrap pd-matrix-wrap live-daily-matrix"><table><thead><tr>'+sorted.headers.map(value=>'<th>'+value+'</th>').join('')+'</tr></thead><tbody>'+body+'</tbody></table>'+(visible.length?'':'<div class="live-empty">当前筛选范围没有已入库记录</div>')+'</div>';
