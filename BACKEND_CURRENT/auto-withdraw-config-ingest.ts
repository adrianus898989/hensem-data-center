...
Deno.serve(async(req:Request)=>{
  ...
  try {
    ...
    const target=await db("ar_config_targets?"+new URLSearchParams({select:"timezone",country_code:"eq."+s.country_code,platform:"eq."+s.platform,limit:"1"}));
    if(target[0]?.timezone!==s.timezone)return reply({ok:false,error:"target_timezone_mismatch"},400);
