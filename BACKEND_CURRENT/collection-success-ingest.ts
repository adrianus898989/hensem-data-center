...
export function validateCollectionSnapshot(value: unknown, now = new Date()): JsonObject {
  ...
  if (value.schema_version !== 1 || value.source_system !== "RECHARGE_REVIEW") invalid("schema_version/source_system");
...
export function createCollectionSuccessHandler(dependencies: Dependencies): (request: Request) => Promise<Response> {
  ...
  return async request => {
    try {
      ...
      if (!credential || credential.source_system !== "RECHARGE_REVIEW" || credential.revoked !== false
      ...
      if (snapshot.source_system !== credential.source_system || !credential.allowed_scopes.some(scope => scope.country_code === snapshot.country_code
