...
test('return control sends only an encoded one-command message and never reads a host session',()=>{
 ...
 vm.runInNewContext(scripts[0],{document,parent:{postMessage:(payload,target)=>messages.push({payload,target})}},{codeGeneration:{strings:false,wasm:false}});
