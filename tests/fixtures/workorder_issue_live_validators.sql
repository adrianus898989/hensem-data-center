-- Read-only live schema/function definitions verified September 19, 2026.
-- Offline test baseline only; no credentials or business rows are included.
  ...
  if p_snapshot->>'source_system' <> 'AR_WORKORDER'
    ...
    or not (
      (
        ...
        and p_snapshot->>'platform' = 'POPZAR'
...
begin
  if pg_catalog.jsonb_typeof(p_scopes) is distinct from 'array'
    or pg_catalog.jsonb_array_length(p_scopes) not between 1 and 2 then
    return false;
  end if;
  ...
  loop
    if v_scope <> '{"country_code":"PK","platform":"POPZAR","timezone":"Asia/Karachi"}'::jsonb
      and v_scope <> '{"country_code":"IN","platform":"DhaniWin","timezone":"Asia/Kolkata"}'::jsonb then
      return false;
    end if;
