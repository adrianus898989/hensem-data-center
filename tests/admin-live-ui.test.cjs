...
test('overview preserves the full reference dashboard and anchor navigation without extra queries',async()=>{
 ...
 const navigation=html.match(/<nav class="df-jumps"[^>]*>([^]*?)<\/nav>/)?.[1];assert(navigation);assert.equal([...navigation.matchAll(/<a\b/g)].length,7);for(const [,id]of navigation.matchAll(/href="#([^"]+)"/g))assert(html.includes('id="'+id+'"'),'anchor '+id+' has a visible destination');assert.equal(h.calls.length,calls);assert.doesNotMatch(html,/NaN|Infinity/);
});
...
test('unconnected modules preserve reference schemas and local controls without inventing source rows or queries',async()=>{
 const h=await ready(),calls=h.calls.length,expected={workorders:['提交金额','提交笔数','到账金额','到账笔数','未到账金额','未到账笔数'],dropped:['订单 ID','订单号','平台','三方','方向','掉单金额','掉单笔数'],anomaly:['异常类型','订单 ID','订单号','平台','三方','方向','异常金额','异常笔数'],events:['事件编号','三方','方向','关联金额','关联笔数','处理状态'],rules:['最低订单量','超期账龄','数据不足处理','多项命中处理'],access:['账号','账号标识','角色','团队','商户（平台）范围'],ip:['IP / CIDR','适用入口','适用账号','状态'],login_logs:['账号','登录时间','IP','登录结果','白名单结果','会话状态'],operation_logs:['操作者','操作时间','动作','资源','请求 ID'],teams:['原始平台','团队','国家','来源','匹配订单数'],provider_config:['原始三方','统一三方','来源','匹配订单数'],platform_systems:['原始平台','包网系统','来源','匹配订单数']};
 for(const [page,fields]of Object.entries(expected)){h.c.setPage(page);const localDraft=['teams','provider_config','platform_systems'].includes(page);if(localDraft)assert.equal(h.html(),'INDEPENDENT_SNAPSHOT',page+' preserves the independent local draft renderer');const html=localDraft?h.c.HensemLiveEmpty.render(page):h.html();assert(html.includes('data-empty-page="'+page+'"'),page);assert.match(html,/未接入/);assert.match(html,/data-empty-field=/);const tables=renderedTables(html),headers=tables.flatMap(t=>t.headers);for(const field of fields)assert(headers.includes(field),page+' preserves '+field);for(const table of tables){assert.equal(table.rows.length,1,page+' only shows the empty row');assert.equal(table.rows[0].length,1,page+' does not fabricate records');assert.match(table.rows[0][0],/未接入/);assert(table.html.includes('colspan="'+table.headers.length+'"'),page+' empty row spans schema')}assert.doesNotMatch(html,/Synthetic provider|order-0|row-0|NaN|Infinity/);assert.equal(h.calls.length,calls,page+' does not query an unrelated order dataset')}
