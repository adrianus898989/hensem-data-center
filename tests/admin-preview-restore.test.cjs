...
test('published overlay matches every reviewed renderer and style source exactly',()=>{
 ...
 assert.equal(m.exports.restoredCss,['live-restored.css','live-reference-pages.css','live-payout-config.css','live-configuration.css','live-sync-health.css'].map(read).join('\n'));
});
