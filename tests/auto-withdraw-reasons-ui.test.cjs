...
function render({expanded=false,inline=true,allowed=true,empty=false,error=''}={}) {
  ...
  const states=[expanded?target:null,inline,expanded?{key:JSON.stringify(target),day:empty?null:day}:null,error,false,0,'manual','',1];
  ...
  new Function('require','module','exports',source)(name=>{
    ...
    if(name==='./DashboardAuthGate')return {useDashboardAuth:()=>({session:{access_token:'test'},profile:{active:allowed,role:'owner'}})};
...
test('expanded reason panel appears below exactly the selected platform, not a modal',()=>{
  ...
  assert.match(html,/合并明细 · 2 组/); assert.match(html,/14 笔/); assert.match(html,/19 笔/);
  assert.match(html,/受限游戏类型投注/); assert.match(html,/100.00%/); assert.match(html,/不是全部订单明细/);
});
