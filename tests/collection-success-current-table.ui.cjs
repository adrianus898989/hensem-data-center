...
const workOrderRows=[
  ['UPI-QR',100,20,100,70],
  ['ArbPay',100,70,100,95],
  ['ICPay',100,95,10,0],
].map(([third_party,submitted_count,success_count,withdraw_not_received_count,withdraw_success_count],index)=>({id:`work-${index}`,system_name:'AR',source_system:'AR_WORKORDER',stat_date:day,country_code:'IN',country:'印度',platform:'91CLUB',third_party,channel_type:'UPI',submitted_amount:1000,submitted_count,success_amount:500,success_count,withdraw_not_received_amount:1000,withdraw_not_received_count,withdraw_success_amount:500,withdraw_success_count}));
...
(async()=>{
  ...
  try {
    ...
    window.fixture={rows:${safe(JSON.stringify(volumeRows))},snapshots:${safe(JSON.stringify(snapshots))},rates:${safe(JSON.stringify(rates))},workOrderRows:${safe(JSON.stringify(workOrderRows))}};
    const qa=require('./ThirdPartyVolumeDashboard').__qa,helper=require('./collectionSuccess'),workHelper=require('./workOrderDeposit');const app=ReactDOM.createRoot(document.getElementById('app'));
    window.render=(enabled=true,mode='complete')=>{const rows=qa.aggregateCombo(window.fixture.rows,r=>[r.country,r.channel]);const feeCombos=qa.aggregateCombo(window.fixture.rows,r=>[r.country,r.platform,r.channel,r.channelType]);const feeRows=qa.buildFeeCompareRows(feeCombos,window.fixture.rates,[],'platform');const selectedSnapshots=mode==='missing'?[]:mode==='partial'?window.fixture.snapshots.filter(s=>!(s.platform==='IN999'&&s.stat_date==='${day}')):window.fixture.snapshots;const view=helper.buildCollectionSuccessView({snapshots:selectedSnapshots,volumeRows:window.fixture.rows,start:'${day}',end:'${day}',country:'印度'});app.render(__jsx(qa.MonthlyTable,{title:'印度线下盘口 汇总',subtitle:'',rows,columns:['国家','统一三方'],feeRows,paginated:true,collectionSuccess:enabled?view:undefined}));};window.render(false);
    window.renderWorkOrders=()=>{const rows=qa.aggregateCombo(window.fixture.rows,r=>[r.country,r.channel]);const workOrderDeposit=workHelper.buildWorkOrderDepositView({rows:window.fixture.workOrderRows,volumeRows:window.fixture.rows,start:'${day}',end:'${day}',country:'印度'});app.render(__jsx(qa.MonthlyTable,{title:'工单成功占比预警验收',subtitle:'',rows,columns:['国家','统一三方'],paginated:true,workOrderDeposit}));};
    </script></body></html>`);
    ...
    await page.evaluate(()=>window.renderWorkOrders());
    await page.getByRole('columnheader',{name:/存款未到账 成功占比/}).waitFor();
